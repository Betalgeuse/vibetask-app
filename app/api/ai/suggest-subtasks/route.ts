...
async function resolveAiLabelId(
  ...
  const aiLabel = labels?.find((label) =>
...
export async function POST(request: Request) {
  ...
  const { projectId, parentId, taskName, description } = body as {
    projectId?: string;
    parentId?: string;
    taskName?: string;
    description?: string;
  ...
  try {
      ...
      messages: [
        {
          role: "system",
          content:
            "Suggest 2 missing sub tasks. Return JSON object with key 'todos', each item having taskName and description.",
        },
        {
          role: "user",
    ...
    const items = parseTodosFromContent(
